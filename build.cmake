function(BUILD)
    ...
    if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${BUILD_TARGET}/include/config.hpp.in)
        ...
        set(CONFIG_HPP_IN ${CMAKE_CURRENT_SOURCE_DIR}/${BUILD_TARGET}/include/config.hpp.in)
        set(INCLUDEDIR "${CMAKE_CURRENT_BINARY_DIR}/.gen/${BUILD_TARGET}/include")
        set(BUILDINFO_HPP ${INCLUDEDIR}/buildinfo.hpp)
        file(MAKE_DIRECTORY ${INCLUDEDIR})
        if(BUILD_HEADER_LIBRARY)
            set(CONFIG_HPP ${INCLUDEDIR}/${BUILD_HEADER_LIBRARY}.config.hpp)
            target_include_directories(${BUILD_TARGET} INTERFACE ${CMAKE_CURRENT_BINARY_DIR}/.gen/${BUILD_TARGET}/include)
        else()
            set(CONFIG_HPP ${INCLUDEDIR}/config.hpp)
        ...
        set(MACRO_LIST "")
        foreach(match IN LISTS MACRO_MATCHES)
            string(REGEX REPLACE "#define +([A-Za-z0-9_]+).*" "\\1" MACRO_NAME "${match}")
        ...
        foreach(MACRO IN LISTS MACRO_LIST)
            set(LINE "{\"${MACRO}\",\n#ifdef ${MACRO}\nSTRINGIFY(${MACRO})\n#else\n\"<UNDEFINED>\"\n#endif\n}")
            if(NOT ${MACRO} STREQUAL ${LAST_ITEM})
                string(APPEND LINE ",\n")
            else()
                string(APPEND LINE "\n")
            endif()
            file(APPEND "${BUILDINFO_HPP}" "${LINE}")
        endforeach()
        file(APPEND "${BUILDINFO_HPP}" "};\n")
    endif()

    # Embed the license file if told to
    if(BUILD_EMBED_LICENSE)
        file(READ ${BUILD_EMBED_LICENSE} licenseContents)
        # Escape backslashes and quotes (just in case)
        string(REPLACE "\\" "\\\\" licenseContents "${licenseContents}")
        string(REPLACE "\"" "\\\"" licenseContents "${licenseContents}")
        # Set file
        set(licenseHeader "${CMAKE_CURRENT_BINARY_DIR}/.gen/${BUILD_TARGET}/include")
        # See if we need extra action
        if(NOT EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${BUILD_TARGET}/include/config.hpp.in)
            # Replicate some functionality used when making a config.hpp file
            file(MAKE_DIRECTORY "${licenseHeader}")
            if(BUILD_HEADER_LIBRARY)
                target_include_directories(${BUILD_TARGET} INTERFACE ${CMAKE_CURRENT_BINARY_DIR}/.gen/${BUILD_TARGET}/include)
            else()
                target_include_directories(${BUILD_TARGET} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}/.gen/${BUILD_TARGET}/include)
        ...
        if(BUILD_HEADER_LIBRARY)
            set(licenseHeader "${licenseHeader}/${BUILD_HEADER_LIBRARY}.license.h")
        else()
            set(licenseHeader "${licenseHeader}/license.h")
        endif()
        # Write the file
        file(WRITE  "${licenseHeader}" "#pragma once\n")
        file(APPEND "${licenseHeader}" "// Auto-generated by cmake-libs::build::embed_license\n")
        file(APPEND "${licenseHeader}" "constexpr const char* LICENSE_TEXT = R\"LICENSE(\n")
        file(APPEND "${licenseHeader}" "${licenseContents}")
        file(APPEND "${licenseHeader}" "\n)LICENSE\";\n")
    endif()
    ...
    if(${BUILD_LTO_TYPE} STREQUAL "thin")
        ...
        target_compile_options(${BUILD_TARGET} PRIVATE -ffat-lto-objects)
