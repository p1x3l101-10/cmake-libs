function(BUILD)
    ...
    if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${BUILD_TARGET}/include/config.hpp.in)
        ...
        list(GET CXX_FLAGS_LIST -1 LAST_ITEM)
        list(LENGTH CXX_FLAGS_LIST LENGTH)
        file(APPEND "${BUILDINFO_HPP}" "constexpr std::array<std::string, ${LENGTH}> compileOptions{\n")
        foreach(FLAG IN LISTS CXX_FLAGS_LIST)
            set(LINE "\"${FLAG}\"")
            if(NOT ${FLAG} STREQUAL ${LAST_ITEM})
                string(APPEND LINE ",\n")
            else()
                string(APPEND LINE "\n")
            endif()
        ...
        list(GET CMAKE_EXE_LINKER_FLAGS_LIST -1 LAST_ITEM)
        list(LENGTH CMAKE_EXE_LINKER_FLAGS_LIST LENGTH)
        file(APPEND "${BUILDINFO_HPP}" "constexpr std::array<std::string, ${LENGTH}> linkOptions{\n")
        foreach(FLAG IN LISTS CMAKE_EXE_LINKER_FLAGS_LIST)
            set(LINE "\"${FLAG}\"")
            if(NOT ${FLAG} STREQUAL ${LAST_ITEM})
                string(APPEND LINE ",\n")
            else()
        ...
        endforeach()
        file(APPEND "${BUILDINFO_HPP}" "};\n")
        file(APPEND "${BUILDINFO_HPP}" "constexpr std::string buildType = \"${CMAKE_BUILD_TYPE}\"")
    endif()
